...
def test_ros_typedb_lc_states(test_node):
    configure_res = test_node.change_ros_typedb_state(1)
    get_inactive_state_res = test_node.get_ros_typedb_state()

    activate_res = test_node.change_ros_typedb_state(3)
    get_active_state_res = test_node.get_ros_typedb_state()
...
@pytest.mark.launch(fixture=generate_test_description)
def test_ros_typedb_cleanup_transition(test_node):
    configure_res = test_node.change_ros_typedb_state(1)
    activate_res = test_node.change_ros_typedb_state(3)
    deactivate_res = test_node.change_ros_typedb_state(4)
    cleanup_res = test_node.change_ros_typedb_state(2)
    get_unconfigured_state_res = test_node.get_ros_typedb_state()

    assert configure_res.success is True
    assert activate_res.success is True
    assert deactivate_res.success is True
    assert cleanup_res.success is True
    assert get_unconfigured_state_res.current_state.id == 1


@pytest.mark.launch(fixture=generate_test_description)
def test_ros_typedb_insert_query(test_node, insert_query):
    test_node.activate_ros_typedb()

    query_res = test_node.call_service(test_node.query_cli, insert_query)
...
def test_ros_typedb_insert_event(insert_query):
    ...
    node.call_service(node.query_cli, insert_query)
    event_flag = node.typedb_event.wait(timeout=5.0)
    assert event_flag and node.typedb_event_data == 'insert'


@pytest.mark.launch(fixture=generate_test_description)
def test_ros_typedb_delete_query(test_node, insert_query):
    test_node.activate_ros_typedb()
...
def test_ros_typedb_delete_event(test_node, insert_query):
    ...
    """
    query_res = test_node.call_service(test_node.query_cli, query_req)
    event_flag = test_node.typedb_event.wait(timeout=5.0)
    assert event_flag and test_node.typedb_event_data == 'delete'

    assert query_res.success is True


@pytest.mark.launch(fixture=generate_test_description)
def test_ros_typedb_wrong_query(test_node, insert_query):
    test_node.activate_ros_typedb()
...
@pytest.mark.launch(fixture=generate_test_description)
def test_ros_typedb_get_aggregate_query(test_node, insert_query):
    test_node.activate_ros_typedb()
...
class MakeTestNode(Node):
    ...
    def __init__(self, name='test_node'):
        ...
        self.delete_db_cli = self.create_client(
            Empty, '/ros_typedb/delete_database'
        )
    ...
    def change_ros_typedb_state(self, transition_id):
        ...
        return self.call_service(self.change_state_cli, change_state_req)

    def get_ros_typedb_state(self):
        get_state_req = GetState.Request()
        return self.call_service(self.get_state_cli, get_state_req)
