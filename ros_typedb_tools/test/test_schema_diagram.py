"""Tests for TypeDB schema diagram parsing and graph generation."""
...
def test_parse_schema_extracts_core_elements():
    ...
    assert [rule.name for rule in schema.rules] == ["active_server"]


def test_graph_contains_expected_edges_and_rule_notes():
    schema = parse_schema_file(_fixture_schema_path())
    graph_dot = build_graph(schema, include_rules=True).to_string()
    ...
    assert "rule__active_server" in graph_dot
...
def test_merge_schema_models_combines_definitions_and_rules():
...
rule validate_node:
when { $x isa Node; } then { $x has name "ok"; };
"""
    )
...
rule validate_node:
when { $x isa Node; } then { $x has enabled true; };
"""
    )
    ...
    assert [rule.name for rule in merged.rules] == ["validate_node"]
...
def test_parse_schema_files_merges_multiple_files(tmp_path: Path):
    ...
    second.write_text("define\nA sub entity, owns x;\nx sub attribute, value string;", encoding="utf-8")
...
"""
    ...
    assert 'shared_relation [label="shared_relation", shape=diamond' in graph_dot
    assert 'style="rounded,filled"' in graph_dot
