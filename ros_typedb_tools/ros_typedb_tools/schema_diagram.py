"""Utilities to parse TypeDB 3 schemas and generate diagram graphs."""
...
class SchemaFunction:
    """Represents a function declared in a TypeDB schema."""

    name: str


SchemaRule = SchemaFunction
...
class SchemaModel:
    ...
    functions: List[SchemaFunction] = field(default_factory=list)

    @property
    def rules(self) -> List[SchemaFunction]:
        """Compatibility alias for older rule-based callers."""
...
def parse_schema_text(text: str) -> SchemaModel:
    ...
    function_names = _extract_function_names(cleaned_text)
    statements = _split_top_level_statements(cleaned_text)

    model = SchemaModel(functions=[SchemaFunction(name=name) for name in function_names])
    for statement in statements:
        ...
        function_or_rule_match = re.match(
            r"^(?:rule|fun)\s+([A-Za-z_][\w-]*)\s*(?::|\()",
            statement,
            flags=re.IGNORECASE,
        )
        if function_or_rule_match:
            continue
        ...
        if type_match:
            ...
            schema_type = model.types.setdefault(
                type_name,
                SchemaType(name=type_name, kind="unknown"),
            )
        ...
        standalone_plays_match = re.match(
            r"^([A-Za-z_][\w-]*)\s+(.*)$",
            statement,
            flags=re.DOTALL,
        )
...
def merge_schema_models(models: Iterable[SchemaModel]) -> SchemaModel:
    ...
    for model in models:
        ...
        for function in model.functions:
            ...
            merged.functions.append(function)
...
def build_graph(
    ...
    if include_functions:
        for function in sorted(schema.functions, key=lambda item: item.name):
            function_node_name = f"function__{function.name}"
            graph.add_node(
                pydot.Node(
                    function_node_name,
                    label=f"fun: {function.name}",
                    shape="note",
                    style="filled",
                    fillcolor="#F7F3D2",
                    color="#C0B66F",
...
def write_graph(
    ...
    graph = build_graph(schema, include_functions=include_functions, orientation=orientation)
...
def _extract_function_names(text: str) -> List[str]:
    names = re.findall(r"(?m)^\s*fun\s+([A-Za-z_][\w-]*)\s*\(", text)
    seen: Set[str] = set()
