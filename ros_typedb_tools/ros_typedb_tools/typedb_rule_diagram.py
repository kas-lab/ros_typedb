"""CLI for generating TypeDB 3 function dependency/read diagrams."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    """Build command-line args for the TypeDB function diagram tool."""
    parser = argparse.ArgumentParser(
        prog="typedb_rule_diagram",
        description="Generate TypeDB 3 function dependency/read diagrams from .tql files.",
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable function-to-function dependency edges.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    """Run the TypeDB function diagram CLI."""
    ...
    if missing_paths:
        print(f"Input TypeDB file not found: {missing_paths[0]}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Failed to generate function diagram: {exc}", file=sys.stderr)
        return 1

    print(f"Wrote function diagram: {output_path}")
    return 0
